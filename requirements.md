...
- [R2] Каналы взаимодействия: веб-интерфейс и Telegram-бот.
...
- [R7] **Интеграции**: Telegram-бот (python-telegram-bot), Email (SMTP - стандартная библиотека smtplib).
...
- [R15] Привязка Telegram через deep-link `/start <one-time token>`
...
### 3.5 Telegram-бот
- [R29] Команды: `/start` (привязка), `/add`, `/list`, `/help`.
- [R30] Уведомления с inline-кнопками: «Продлить (+1м/+1г/Custom)», «Snooze 1 день», «Отменить», «Открыть в веб-UI».
- [R31] Вебхук на бэкенде с валидацией.
